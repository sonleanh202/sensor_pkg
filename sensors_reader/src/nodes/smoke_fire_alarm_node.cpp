...
#include "sensors_reader/nodes/smoke_fire_alarm_node.hpp"
...
SensorDefaults make_smoke_defaults()
{
  ...
  defaults.unit = "state";
  defaults.topic_name = "/sensor/smoke_status";
...
int main(int argc, char ** argv)
{
  ...
  auto node = std::make_shared<sensors_reader::SmokeFireAlarmNode>();
  rclcpp::spin(node);
  rclcpp::shutdown();
  return 0;
}
