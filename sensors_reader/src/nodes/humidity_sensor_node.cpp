#include "sensors_reader/nodes/humidity_sensor_node.hpp"
...
: Node("humidity_sensor_node")
{
  ...
  this->declare_parameter<std::string>("humidity_topic_name", "/sensor/humidity");
  this->declare_parameter<std::string>("temperature_topic_name", "/sensor/temp");
...
int main(int argc, char ** argv)
{
  ...
  auto node = std::make_shared<sensors_reader::HumiditySensorNode>();
  rclcpp::spin(node);
  rclcpp::shutdown();
  return 0;
}