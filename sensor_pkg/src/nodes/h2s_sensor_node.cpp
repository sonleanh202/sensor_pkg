  ...
  passive_mode_configured_(false)
{
  ...
  this->declare_parameter<int>("poll_interval_ms", 10000);
  this->declare_parameter<int>("startup_delay_ms", 500);
  ...
  this->declare_parameter<std::string>(
    "notes",
    "DFRobot SEN0467 UART protocol sharing one RS485 bus");
  ...
  // Do the first H2S transaction almost immediately. This is important because
  // ROS timers normally fire after the full poll interval; with poll_interval_ms=10000,
  // H2S would otherwise not enter passive mode until 10 seconds after launch,
  // while Modbus nodes may already be polling the shared /dev/ttyUSB0 bus.
  startup_timer_ = this->create_wall_timer(
    std::chrono::milliseconds(startup_delay_ms_),
    std::bind(&H2S_SensorNode::startup_callback, this));
}

void H2S_SensorNode::startup_callback()
{
  if (startup_timer_) {
...
void H2S_SensorNode::start_periodic_timer()
{
...
std::string H2S_SensorNode::evaluate_status(double ppm) const
{
  if (alarm_threshold_ >= 0.0 && ppm >= alarm_threshold_) {
    return "ALARM";
  }

  if (warning_threshold_ >= 0.0 && ppm >= warning_threshold_) {
    return "WARN";
  }
...
void H2S_SensorNode::timer_callback()
{
  ...
  try {
    ...
    if (!passive_mode_configured_) {
      ...
      RCLCPP_INFO(
        this->get_logger(),
        "%s | passive mode configured",
        sensor_name_.c_str());
    }

    const double ppm = client_->read_h2s_ppm(static_cast<uint8_t>(address_));
    ...
    msg.value = ppm;
    msg.unit = unit_;
    msg.raw_value = static_cast<int>(ppm);
    msg.alarm = alarm;
