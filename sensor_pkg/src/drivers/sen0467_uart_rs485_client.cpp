...
Sen0467UartRs485Client::Sen0467UartRs485Client(
...
void Sen0467UartRs485Client::connect()
{
  if (is_connected()) {
    return;
  }
...
void Sen0467UartRs485Client::disconnect()
{
  if (fd_ >= 0) {
    ::close(fd_);
    fd_ = -1;
...
bool Sen0467UartRs485Client::is_connected() const
{
  return fd_ >= 0;
}

void Sen0467UartRs485Client::open_port()
{
...
void Sen0467UartRs485Client::configure_port()
{
  struct termios tty;
...
void Sen0467UartRs485Client::flush_io()
{
  if (fd_ >= 0) {
...
void Sen0467UartRs485Client::write_frame(const std::array<uint8_t, 9> & frame)
{
...
std::array<uint8_t, 9> Sen0467UartRs485Client::read_frame()
{
...
speed_t Sen0467UartRs485Client::baud_to_speed(int baudrate)
{
  switch (baudrate) {
    case 9600:
      return B9600;
...
uint8_t Sen0467UartRs485Client::calc_checksum(const std::array<uint8_t, 9> & frame)
{
  uint8_t sum = 0;
...
std::array<uint8_t, 9> Sen0467UartRs485Client::build_frame(
  uint8_t address,
  uint8_t command,
  uint8_t data3,
  uint8_t data4,
...
void Sen0467UartRs485Client::validate_checksum(const std::array<uint8_t, 9> & frame)
{
...
void Sen0467UartRs485Client::set_passive_mode(uint8_t address)
{
  if (!is_connected()) {
    connect();
  }
...
int Sen0467UartRs485Client::read_h2s_ppm(uint8_t address)
{
  if (!is_connected()) {
    connect();
  }
